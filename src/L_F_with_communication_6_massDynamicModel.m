%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MODEL PREDICTIVE CONTROL: AUTONOMOUS CARS WITH COMUNICATION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% the former (original) file is: "L_F_without_communication_6.m"
% the next file is: "" ==> goal:
% working on this file started  : 3/19/2018
% working on this file Finished : 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function L_F_with_communication_6_massDynamicModel
...
%%%%%%  LEADER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    x0_L(:,1,1)=  [ 10    ; 55     ; 0     ; 0    ]; % expected value of the initial position for the Leaders vehicle itself
    x0_L(:,1,2)=  [ 0.03  ; 0.03  ; 0.02  ; 0.02  ]; % variance of x0_L 
...
%          100   127  127];
    ...
    x_L = zeros(length(x0_L(:,1,1)),mpciterations+1,2);% the 1st page show "mu", and the 2nd page shows "var"
    
    x_L(:,1,1)=x0_L(:,1,1); % substitute the mu  of the initial state of the Leader
    x_L(:,1,2)=x0_L(:,1,2); % substitute the var of the initial state of the Leader
...
%%%%%%  FOLLOWER  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    x_curr_L_measuredByF(:,1,1)=  x0_L(:,1,1)               ; % expected value for x0_L measured by the Follower
    x_curr_L_measuredByF(:,1,2)= [0.05; 0.05; 0.03 ; 0.03]; % variance of x0_L measured by Follower    
    % x_curr_L_measuredByF(:,1,1): (mu)  shows the "expected values" of the Leader's state variables measured by the Follower
    % x_curr_L_measuredByF(:,1,2): (var) shows the       "variances" of the Leader's state variables measured by the Follower
    ...
    u0_F(1,:)=0*ones(1,N);% throttle
    %u0_F(2,:)=0 + (0.1*dDelta_max_F-0).*ones(1,N);% rate of changes in steering angle
    u0_F(2,:)=0*ones(1,N); % brake
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    t0= 0;                       % starting time
    t = zeros(1,mpciterations+1);% time steps
    t(1,1)=t0;% it puts the zero time (t0) as the current time step (t_curr)
    ...
    %%%%%% Leader %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       %}  
        ...
        % in this phase, we just apply the first "applied_steps" steps and do the
        % optimization again. this can be changed.
    ...
    %%%%%% Follower %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        policy_L_transmittedToF{1,1}=zeros(length(x0_L(:,1,1)),N,2); % the states of the Leader for N time steps in the future that are transmitted to the Follower
        policy_L_transmittedToF{2,1}=u_new_L; % the input control of the Leader for N time steps in the future that are transmitted to the Follower
        
        for counter=1:N
            
            if counter==1
                position_of_Follower=x_L(:,i,:);
            else
                position_of_Follower=policy_L_transmittedToF{1,1}(:,counter-1,:);
            end
            policy_L_transmittedToF{1,1}(:,counter,:) =  SystemModel_L(t_curr, ...
                position_of_Follower, policy_L_transmittedToF{2,1}(:,counter),dt); % apply control for as many as applied_steps
        end
        ...
        x_curr_L_measuredByF(:,1,2)=x_curr_L_measuredByF(:,1,2); % var of the current state of the Leader that has been measured by the Follower
        
        x_curr_F=x_F(:,i-1,:); % current state of Follower
        [u_new_F,obj_F] = OptimalControl_F(@runningcosts_F, @terminalcost_F, @constraints_F, ...
                                  @terminalconstraints_F, @linearconstraints_F, ...
                                  @SystemModel_F, N, t_curr, x_curr_F, u0_F, dt, x_curr_L_measuredByF, policy_L_transmittedToF); 
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for j=0:applied_steps-1
            t(1,i+j) = t(1,i+j-1)+dt; % calculates the next time step and save them in "t" matrix
        end
...
%%%%%% Print the Results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    filename='L_F_with_communication_6_massDynamicModel.txt';
...
%%%%%% Plot the Results %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    axis square
    axis([0 97 40 137])
    title('Vehicles WITH Communication')% title
    xlabel('X') % x-axis label
    ylabel('Y') % y-axis label
    %legend('Reference Path','Leader','Follower','Location','northwest')% legend
    
    
    x_L_plot=zeros(length(x0_L(:,1,1)),mpciterations*plot_resolution,2);% Leader's info to plot
    x_L_plot(:,1,:)=x0_L(:,1,:);% the initial state of the Leader
    ...
    x_L_plot(:,:,2)=[]; % delete the 2nd page that shows the variance. we don't need it for ploting
    x_F_plot(:,:,2)=[]; % delete the 2nd page that shows the variance. we don't need it for ploting
    
    
    vidfile = VideoWriter('L_F_with_communication_6_massDynamicModel.mp4','MPEG-4');
    vidfile.FrameRate = 5; % How many frames per second.
    open(vidfile); 
    
    subplot(3,2,[1 3])
    hold on
    title('Vehicles With Communication')
...
function x_L = computeOpenloopSolution_L(SystemModel_L, N, dt, t_curr, x_curr_L, u_L)
    x_L(:,1,1) = x_curr_L(:,1,1);
    for k=1:N
        x_L(:,k+1,1) = SystemModel_L(t_curr, x_L(:,k,1), u_L(:,k), dt);
    end
end
...
function cost_L = costfunction_L(runningcosts_L, terminalcosts_L, SystemModel_L, ...
    ...
    x_L(:,:,1) = computeOpenloopSolution_L(SystemModel_L, N, dt, t_curr, x_curr_L, u_L);
    for k=1:N
        cost_L = cost_L+runningcosts_L(t_curr+(k-1)*dt, x_L(:,k,1), u_L(:,k));
    end
    cost_L = cost_L+terminalcosts_L(t_curr+(N+1)*dt, x_L(:,N+1,1));
end
...
function cost_L = runningcosts_L(t,x_L, u_L)
    ...
    distance=point_to_line(x_L(1:2,1,1),xr);
    cost_L = alpha1_L*(abs((x_L(1,1,1)-xg(1)))+abs((x_L(2,1,1)-xg(2))))+...
                                                        alpha3_L*distance;
         %sqrt(((x_L(1,1,1)-xg(1)))^2+((x_L(2,1,1)-xg(2)))^2)+...
    %alpha2_L*u_L(1);
...
function tcost_L=terminalcost_L(t,x_L)
    %global xg alpha4_L
    tcost_L=0;%alpha4_L*(sqrt((x_L(1,1,1)-xg(1))^2+(x_L(2,1,1)-xg(2))^2));%+ abs(x_L(3,1,1)-xg(3))
end
...
function [c_L,ceq_L] = constraints_L(t,x_L, u_L)
    ...
    c_L(1)   = (x_L(3,1,1)^2+x_L(4,1,1)^2-(v_max_L)^2);%[]; this means that the velocity of the vehicle can not be greater than v_max
    %c_L(2)   = (x_L(7,1,1)-steering_ang_max_L);%x_L(7,1,1)<=steering_ang_max_L
...
%     c_L(5)  =  1-abs(((x_L(1,1,1)-74.5)/125)+((x_L(2,1,1)-62.5)/125))-abs(((x_L(1,1,1)-74.5)/125)-((x_L(2,1,1)-62.5)/125));
    
    % obstacle avoidance constraints. Pr(collision)< 1-CL
    exchange_5=length(c_L);
    if (x_L(2,1,1)<125)
          c_L(exchange_5+1) = 8-x_L(1,1,1);
          c_L(exchange_5+2) = x_L(1,1,1)-12;
    else
          c_L(exchange_5+1) = 8-x_L(1,1,1);
          c_L(exchange_5+2) = x_L(2,1,1)-129;
    end

    if (x_L(1,1,1)>12)
          c_L(exchange_5+3) = 125-x_L(2,1,1);
          c_L(exchange_5+4) = x_L(2,1,1)-129;
    else
          c_L(exchange_5+3) = 8-x_L(1,1,1);
          c_L(exchange_5+4) = x_L(2,1,1)-129;
    end
...
function x_L = SystemModel_L(t,x_L, u_L, dt)
    ...
    x_L(3,1,1)= x_L(3,1,1) + u_L(1)*dt; % velocity of the Leader at x-coordinate
    x_L(4,1,1)= x_L(4,1,1) + u_L(2)*dt; % velocity of the Leader at y-coordinate 
       
    x_L(1,1,1)=x_L(1,1,1)+dt*x_L(3,1,1); % x-coordinate of the Leader (new)
    x_L(2,1,1)=x_L(2,1,1)+dt*x_L(4,1,1); % y-coordinate of the Leader (new)
...
function [u_new_F,v_cost_F]=OptimalControl_F(runningcosts_F, terminalcosts_F, ...
                             ...
                             linearconstraints_F, SystemModel_F, N, t_curr, x_curr_F, u0_F, dt,x_curr_L_measuredByF, policy_L_transmittedToF)
    ...
    for k=1:N
        [Anew_F, bnew_F, Aeqnew_F, beqnew_F, lbnew_F, ubnew_F] = ...
               linearconstraints_F(t_curr+(k-1)*dt,x_F(:,k,:),u0_F(:,k),x_curr_L_measuredByF, policy_L_transmittedToF);
    ...
    [u_new_F, v_cost_F] = fmincon(...
        @(u_F) costfunction_F(runningcosts_F,terminalcosts_F, SystemModel_F, N, dt, t_curr, x_curr_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)...
               , u0_F, A_F, b_F, Aeq_F, beq_F,lb_F, ub_F, ...
        @(u_F) nonlinearconstraints_F(constraints_F, terminalconstraints_F, SystemModel_F, N, dt, t_curr, x_curr_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)...
               ,options);
...
function cost_F = costfunction_F(runningcosts_F, terminalcosts_F, SystemModel_F, N, dt, t_curr, x_curr_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)
    cost_F = 0;
    %x_F = zeros(N+1, length(x_curr_F));
    x_F = computeOpenloopSolution_F(SystemModel_F, N, dt, t_curr, x_curr_F, u_F);
    for k=1:N
        cost_F = cost_F+runningcosts_F(t_curr+(k-1)*dt, x_F(:,k,:), u_F(:,k),x_curr_L_measuredByF, policy_L_transmittedToF{1,1}(:,k,:));
    end
    cost_F = cost_F+terminalcosts_F(t_curr+(N+1)*dt, x_F(:,N+1,:),x_curr_L_measuredByF, policy_L_transmittedToF{1,1}(:,end,:));
end

function [c_F,ceq_F] = nonlinearconstraints_F(constraints_F,terminalconstraints_F,SystemModel_F, ...
                       N, dt, t_curr, x_curr_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)
    %x_F = zeros(N+1, length(x_curr_F));
    x_F = computeOpenloopSolution_F(SystemModel_F, N, dt, t_curr, x_curr_F, u_F);
    c_F = [];
    ceq_F = [];
    for k=1:N
        [cnew_F, ceqnew_F] = constraints_F(t_curr+(k-1)*dt,x_F(:,k,:),u_F(:,k),x_curr_L_measuredByF, policy_L_transmittedToF);
        c_F = [c_F cnew_F];
        ceq_F = [ceq_F ceqnew_F];
    end
    [cnew_F, ceqnew_F] = terminalconstraints_F(t_curr+(N+1)*dt,x_F(:,N+1,:),x_curr_L_measuredByF, policy_L_transmittedToF);
    c_F = [c_F cnew_F];
    ceq_F = [ceq_F ceqnew_F];
end


function cost_F = runningcosts_F(t,x_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)

    global  critical_distance alpha1_F alpha4_F big_M TTC_min dt safety_distance virtual_lead_line alpha5_F alpha2_F %alpha3_F
    % policy_L_transmittedToF in this function is a matrix
    % [length(x0_L),1,2] and show the expected location of Leader at specific 
    % time step that has been calculated and sent by Leader to Follower
    %{
    % time to collision
    ttc=(sqrt(((policy_L_transmittedToF(1,1,1)-x_F(1,1,1)))^2+((policy_L_transmittedToF(2,1,1)-x_F(2,1,1)))^2))/...
        (sqrt((policy_L_transmittedToF(3,1,1))^2+(policy_L_transmittedToF(4,1,1))^2)-sqrt((x_F(3,1,1))^2+(x_F(4,1,1))^2));
    ...
    currentDistance=sqrt(((policy_L_transmittedToF(1,1,1)-x_F(1,1,1)))^2+((policy_L_transmittedToF(2,1,1)-x_F(2,1,1)))^2);
...
%     safety_distance_penalty=safety_distance/(Manhattan_distance_F_L-critical_distance);
    ...
    Manhattan_distance_F_L=abs(policy_L_transmittedToF(1,1,1)-x_F(1,1,1)) + abs(policy_L_transmittedToF(2,1,1)-x_F(2,1,1));
...
function tcost_F=terminalcost_F(t,x_F,x_curr_L_measuredByF,policy_L_transmittedToF)
    global alpha5_F
    % this cost is zero for now
    tcost_F=0;%*alpha5_F*((norm(policy_L_transmittedToF{1,1}(1,1,1)-x_F(1,1,1))+norm(policy_L_transmittedToF{1,1}(2,1,1)-x_F(2,1,1))));
end

function [c_F,ceq_F] = constraints_F(t,x_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)
...
%     end    
    ...
    % meduim_M=1;
%     c_F(1+meduim_M)=sqrt((x_F(1,1,1)-policy_L_transmittedToF{1,1}(1,1,1))^2+...
%         (x_F(2,1,1)-policy_L_transmittedToF{1,1}(2,1,1))^2)-safety_distance;% this can be deleted
...
function [c_F,ceq_F] = terminalconstraints_F(t,x_F, x_curr_L_measuredByF,policy_L_transmittedToF)
    global mpciterations dt
    c_F   = [];
    ceq_F = [];
end

function [A_F, b_F, Aeq_F, beq_F, lb_F, ub_F] = linearconstraints_F(t,x_F, u_F,x_curr_L_measuredByF, policy_L_transmittedToF)
...
% results. 
global mpciterations dt safety_distance
